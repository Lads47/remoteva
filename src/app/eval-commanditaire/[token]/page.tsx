...
// Formulaire public de satisfaction commanditaire/entreprise (Qualiopi ind. 2),
// identifié via magic-token. Décalque du formulaire d'éval à froid, adressé au
// référent de l'entreprise qui a inscrit ses salariés.
...
export default function PublicSponsorEvalPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = use(params);
  const [data, setData] = useState<SponsorEvalData | null>(null);
  ...
  useEffect(() => {
    ...
    apiFetch<SponsorEvalData>(`/api/public/sponsor-eval/${encodeURIComponent(token)}`, { signal: ac.signal })
      .then((d) => setData(d))
      .catch((err) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await apiFetch(`/api/public/sponsor-eval/${encodeURIComponent(token)}`, {
        method: "POST",
        body: { answers },
      });
      setDone(true);
      ...
      <div className="min-h-screen flex items-center justify-center p-6" style={{ backgroundColor: "#f8fafc" }}>
        <div className="max-w-xl bg-white rounded-2xl shadow-sm border p-8 text-center" style={{ borderColor: "#e5e7eb" }}>
          ...
          <h1 className="text-2xl font-bold" style={{ color: "#1f2244" }}>
            {done ? "Merci pour votre retour ✓" : "Questionnaire déjà soumis"}
          </h1>
          <p className="mt-3 text-sm font-jetbrains" style={{ color: "#727485" }}>
            {done
              ? "Votre appréciation en tant qu'entreprise nous aide à améliorer la qualité de nos formations."
  ...
  const greeting = data.contact.contactName?.trim() || data.contact.companyName?.trim() || "";
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8 text-center">
          <p className="text-xs uppercase tracking-widest font-jetbrains" style={{ color: "#7dcef5" }}>
            Satisfaction entreprise · ≈ 3 minutes
          </p>
        ...
        <div className="mb-6 p-4 rounded-xl border flex items-start gap-3" style={{ borderColor: "#bae6fd", backgroundColor: "#f0f9ff" }}>
          ...
          <p className="text-sm font-jetbrains" style={{ color: "#075985" }}>
            <strong>Bonjour{greeting ? ` ${greeting}` : ""}</strong>, en tant qu&apos;entreprise ayant inscrit
            un ou plusieurs salariés, votre retour sur cette formation (organisation, adéquation à vos besoins,
            impact) nous aide à progresser. Ce questionnaire prend ≈ 3 minutes.
          </p>
        </div>
...
// ============================================================================
// Composants de formulaire (identiques aux autres questionnaires)
// ============================================================================
  ...
  return (
    <div className="bg-white rounded-2xl border p-6 shadow-sm" style={{ borderColor: "#e5e7eb" }}>
      <div className="mb-4">
        ...
        {subtitle && <p className="text-xs font-jetbrains mt-1" style={{ color: "#727485" }}>{subtitle}</p>}
      </div>
      {children}
    </div>
  );
  ...
  return (
    <div>
      ...
      {question.description && (
        <p className="text-xs font-jetbrains -mt-1 mb-2" style={{ color: "#9ca3af" }}>{question.description}</p>
      )}

      {question.type === "likert_5" && (
        <LikertField value={value} onChange={onChange} leftLabel={question.leftLabel} rightLabel={question.rightLabel} />
      )}
      {question.type === "scale_nps" && (
        <NpsField value={value} onChange={onChange} leftLabel={question.leftLabel} rightLabel={question.rightLabel} />
      )}
      ...
      {question.type === "single_choice" && question.options && (
        <div className="space-y-2">
          {question.options.map((opt) => (
            <RadioCard key={opt} active={value === opt} onClick={() => onChange(value === opt ? "" : opt)} label={opt} />
          ))}
        </div>
      )}
      {question.type === "text" && (
...
function RadioCard({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button
      ...
      style={{ borderColor: active ? "#7dcef5" : "#e5e7eb", backgroundColor: active ? "#f0f9ff" : "white" }}
    >
      <span
      ...
      <span className="flex-1 text-sm" style={{ color: "#1f2244" }}>{label}</span>
    </button>
  );
}
...
function YesNoOption({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button
      ...
      style={{ borderColor: active ? "#7dcef5" : "#e5e7eb", backgroundColor: active ? "#f0f9ff" : "white", color: "#1f2244" }}
    >
      {label}
    </button>
  );
