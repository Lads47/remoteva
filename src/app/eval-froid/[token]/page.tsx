...
// Page publique du formulaire d'évaluation à froid (identifié via magic-token).
// Design aligné sur le formulaire d'éval à chaud (et donc inscription) avec
...
export default function PublicColdEvalPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = use(params);
  const [data, setData] = useState<ColdEvalData | null>(null);
      ...
      <div className="min-h-screen flex items-center justify-center p-6" style={{ backgroundColor: "#f8fafc" }}>
        <div className="max-w-xl bg-white rounded-2xl shadow-sm border p-8 text-center" style={{ borderColor: "#e5e7eb" }}>
          ...
          <h1 className="text-2xl font-bold" style={{ color: "#1f2244" }}>
            {done ? "Merci pour vos réponses ✓" : "Questionnaire déjà soumis"}
          </h1>
          <p className="mt-3 text-sm font-jetbrains" style={{ color: "#727485" }}>
            {done
              ? "Vos retours sur l'impact de la formation dans votre poste sont précieux pour nous."
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-6 p-4 rounded-xl border flex items-start gap-3" style={{ borderColor: "#bae6fd", backgroundColor: "#f0f9ff" }}>
          ...
          <p className="text-sm font-jetbrains" style={{ color: "#075985" }}>
            <strong>Bonjour {data.trainee.prenom}</strong>, ce questionnaire prend ≈ 4 minutes.
            Plusieurs semaines après la formation, vos retours nous aident à mesurer son <strong>impact dans votre poste</strong>
            {" "}et à améliorer nos contenus pour les prochains stagiaires.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-xs text-center font-jetbrains" style={{ color: "#9ca3af" }}>
            Vos données sont utilisées uniquement pour évaluer l&apos;impact pédagogique
            et améliorer nos formations, conformément au RGPD.
          </p>
        </form>
      </div>
    </div>
...
// ============================================================================
// Composants réutilisables (identiques au formulaire éval à chaud)
// ============================================================================
