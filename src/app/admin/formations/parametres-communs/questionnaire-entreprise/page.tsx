...
// Éditeur du questionnaire de satisfaction commanditaire/entreprise (global).
// Réutilise le composant SatisfactionWysiwygEditor.
...
export default function SponsorEvalConfigPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<ConfigData>("/api/admin/sponsor-eval-config", { signal: ac.signal })
      .then((d) => {
        setData(d);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const d = await apiFetch<{ questions: SatisfactionQuestion[] }>("/api/admin/sponsor-eval-config", {
        method: "PUT",
        body: { questions },
      });
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm font-jetbrains mt-1" style={{ color: "#727485" }}>
          Envoyé automatiquement au <strong>référent de l&apos;entreprise</strong> qui a inscrit ses salariés,
          <strong> en même temps que l&apos;évaluation à froid</strong> des stagiaires, avec relances à J+7 et J+14.
          Couvre le recueil des appréciations des parties prenantes (Qualiopi indicateur 2).
          Une enquête par entreprise et par session.
        </p>
      </div>
