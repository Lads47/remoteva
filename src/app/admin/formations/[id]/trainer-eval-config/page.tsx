...
export default function FormationTrainerEvalConfigPage({
  ...
  useEffect(() => {
    ...
    apiFetch<ConfigData>(`/api/admin/formations/${id}/trainer-eval-config`, { signal: ac.signal })
      .then((d) => {
        setData(d);
        if (d.override) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const d = await apiFetch<{ override: SatisfactionQuestion[] }>(`/api/admin/formations/${id}/trainer-eval-config`, {
        method: "PUT",
        body: { questions },
      });
      setUsingOverride(true);
  ...
  async function handleResetToGlobal() {
    ...
    setError("");
    try {
      await apiFetch(`/api/admin/formations/${id}/trainer-eval-config`, {
        method: "PUT",
        body: { questions: null },
      });
      setUsingOverride(false);
