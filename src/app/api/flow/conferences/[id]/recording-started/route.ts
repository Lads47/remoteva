...
import { getConference, markRecordingStarted } from "@/lib/conference";
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const conference = await markRecordingStarted(id);
    const projectStatus = await refreshProjectStatus(conference.flowProjectId);

    return jsonCors({ conference, projectStatus });
  } catch (error) {
    console.error("[/api/flow/conferences/:id/recording-started] error:", error);
