// Override du questionnaire d'éval à FROID au niveau d'une formation.
// Jumeau de /api/admin/formations/[id]/satisfaction-config (chaud).
...
export async function GET(
  ...
  try {
    ...
    const f = await prisma.formation.findUnique({
      where: { id },
      select: { id: true, code: true, nomLong: true, coldEvalConfigForm: true },
    });
    ...
    const global = await getGlobalColdEvalQuestions();
    return NextResponse.json({
      formation: { id: f.id, code: f.code, nomLong: f.nomLong },
      override,
      global,
      defaults: COLD_EVAL_DEFAULT_QUESTIONS,
    });
  } catch (error) {
    console.error("[/api/admin/formations/[id]/cold-eval-config] GET error:", error);
...
export async function PUT(
  ...
  try {
    ...
    await prisma.formation.update({
      where: { id },
      data: { coldEvalConfigForm: JSON.stringify(validated) },
    });
    return NextResponse.json({ success: true, override: validated });
  } catch (error) {
    console.error("[/api/admin/formations/[id]/cold-eval-config] PUT error:", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
