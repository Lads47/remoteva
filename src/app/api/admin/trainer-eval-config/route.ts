// API admin : configuration des questions d'éval formateur globales.
...
export async function GET() {
  ...
  try {
    const questions = await getGlobalTrainerEvalQuestions();
    return NextResponse.json({ questions, defaults: TRAINER_EVAL_DEFAULT_QUESTIONS });
  } catch (error) {
    console.error("[/api/admin/trainer-eval-config] GET error:", error);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await setGlobalTrainerEvalQuestions(validated);
    return NextResponse.json({ success: true, questions: validated });
  } catch (error) {
    console.error("[/api/admin/trainer-eval-config] PUT error:", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
