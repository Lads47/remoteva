// API admin : configuration des questions d'évaluation à chaud globales.
...
import {
  ...
  setGlobalSatisfactionQuestions,
...
export async function GET() {
  ...
  try {
    const questions = await getGlobalSatisfactionQuestions();
    return NextResponse.json({ questions, defaults: DEFAULT_QUESTIONS });
  } catch (error) {
    console.error("[/api/admin/satisfaction-config] GET error:", error);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // Vérifie l'unicité des `name` pour les questions vraiment éditables.
    const names = new Set<string>();
    for (const q of validated) {
      if (q.type === "section_header") continue; // OK d'avoir plusieurs section_header avec name distinct
      if (names.has(q.name)) {
        return NextResponse.json({ error: `Le 'name' "${q.name}" est utilisé plusieurs fois.` }, { status: 400 });
      }
      names.add(q.name);
    }
    await setGlobalSatisfactionQuestions(validated);
    return NextResponse.json({ success: true, questions: validated });
  } catch (error) {
    console.error("[/api/admin/satisfaction-config] PUT error:", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
