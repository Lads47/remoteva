// API admin : configuration des questions de satisfaction commanditaire/entreprise.
// Jumeau de /api/admin/cold-eval-config.

import { NextRequest, NextResponse } from "next/server";
import { getSession } from "@/lib/auth";
import {
  SPONSOR_EVAL_DEFAULT_QUESTIONS,
  getGlobalSponsorEvalQuestions,
  setGlobalSponsorEvalQuestions,
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ questions, defaults: SPONSOR_EVAL_DEFAULT_QUESTIONS });
  } catch (error) {
    console.error("[/api/admin/sponsor-eval-config] GET error:", error);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await setGlobalSponsorEvalQuestions(validated);
    return NextResponse.json({ success: true, questions: validated });
  } catch (error) {
    console.error("[/api/admin/sponsor-eval-config] PUT error:", error);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
